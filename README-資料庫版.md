...
Body: { 物件資料 }
```
目前正式資料源為 Supabase，物件新增請使用 `property-admin-db.html` 後台。
...
Body: { 物件資料 }
```
目前正式資料源為 Supabase，物件更新請使用 `property-admin-db.html` 後台。

### 刪除物件（已停用）
```
DELETE /api/properties/:id
```
目前正式資料源為 Supabase，物件刪除請使用 `property-admin-db.html` 後台。

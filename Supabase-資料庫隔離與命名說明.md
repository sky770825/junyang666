...
- **做法**：本專案（濬瑒物件網站 / junyang666）**專用一個 Supabase 專案**。
- **檢查**：
...
- [ ] **專案專用**：此 Supabase 專案（Project Ref: `cnzqtuuegdqwkgvletaa`）僅供「濬瑒物件網站 / junyang666」使用，未與其他專案或網站共用。
- [ ] **名稱唯一**：在 Supabase Dashboard 中，專案顯示名稱已設為易辨識的專用名稱（如 `junyang666` 或 `濬瑒物件網站`）。

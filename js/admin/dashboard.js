...
function showCuteModal(type, title, message, url = null) {
    ...
    modal.innerHTML = `
        ...
        <div class="cute-modal-content">
            ...
            <div class="cute-modal-title">${escapeHtml(title)}</div>
            <div class="cute-modal-message">${escapeHtml(message)}</div>
            ${url ? `<div class="cute-modal-url">${escapeHtml(url)}</div>` : ''}
            <div class="cute-modal-buttons">
                ${url ? `
                    <button class="cute-modal-btn cute-modal-btn-primary" data-url="${escapeHtml(url)}">
                        📋 再次複製
                    </button>
                ` : ''}
...
async function loadRelatedLinks() {
    ...
    try {
        ...
        if (!links || links.length === 0) {
            ...
            links = [
                { id: '09f06dfd-1fba-4911-953c-f9be80363df8', title: '更多物件資料', url: 'https://realtor.houseprice.tw/agent/buy/0928776755/', icon: '🏠', color_gradient: 'linear-gradient(45deg, #2ecc71, #27ae60)', display_order: 1, is_active: true, link_type: 'button' },
                { id: '79abfd24-22c0-470c-9737-6d9bf6b0f110', title: 'TikTok短影音', url: 'https://www.tiktok.com/@aihouse168', icon: '🎵', color_gradient: 'linear-gradient(45deg, #000000, #333333)', display_order: 2, is_active: false, link_type: 'button' },
                ...
                { id: 'ec5964bf-a1f0-48d2-95bf-99e527b96ea0', title: '預售團購區', url: 'https://salersteam.pages.dev/junyang', icon: '🏢', color_gradient: 'linear-gradient(45deg, #9b59b6, #8e44ad)', display_order: 4, is_active: false, link_type: 'button' },

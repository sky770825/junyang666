...
function initFormProgress() {
    ...
    if (form) {
        ...
        function updateProgress() {
            if (window.innerWidth > 768) return; // 只在手機版顯示
            ...
            if (progressText) {
                if (percentage === 100) {
                    progressText.textContent = '✅ 所有必填欄位已填寫完成！';
                    progressText.style.color = '#28a745';
                } else {
                    const remaining = totalRequired - filledCount;
                    progressText.textContent = `還需填寫 ${remaining} 個必填欄位`;
...
async function loadProperties() {
    ...
    try {
        ...
        if (!properties || properties.length === 0) {
            listContainer.innerHTML = '<p style="text-align: center; color: #666; padding: 2rem;">目前沒有物件資料</p>';
            return;
        }
        ...
        listContainer.innerHTML = properties.map((prop, index) => {
            ...
            const publishedStatus = prop.is_published ? '✅ 上架' : '❌ 下架';
            ...
            const sourceLabel = isExternal 
                ? '<span style="background: #ff9800; color: white; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; margin-right: 0.5rem;">🏢 非本店</span>' 
                : '<span style="background: #28a745; color: white; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; margin-right: 0.5rem;">🏠 本店</span>';
            
            return `
            <div class="property-item">
                <div class="property-info">
                    <h4>${sourceLabel}${prop.title || '未命名物件'}</h4>
                    <p>編號: ${prop.number || 'N/A'} | 房型: ${prop.type || 'N/A'} | 價格: ${prop.price || 'N/A'}</p>
                ...
                <div class="property-actions">
                    <button class="btn-small btn-primary" onclick="editProperty('${prop.id}')">✏️ 編輯</button>
                    ...
                    <button class="btn-small btn-danger" onclick="deleteProperty('${prop.id}')">🗑️ 刪除</button>
                </div>
            </div>
        `;
        }).join('');

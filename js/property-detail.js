...
function parsePingAmount(value) {
    const raw = String(value || '').replace(/,/g, '').trim();
    const number = parseFloat(raw.replace(/[^\d.]/g, ''));
...
function estimateDetailMonthlyPayment(property) {
    const priceWan = parseWanAmount(property.price);
    if (!priceWan) return '';
...
function buildDetailSearchText(property, features, transportation) {
    const transportationText = transportation && typeof transportation === 'object'
        ? Object.values(transportation).flat().join(' ')
        : transportation;
...
function hasDetailParking(property, text) {
    return /車位|平車|坡平|機械車|車庫|雙車|停車/.test([
        property.parking_type,
        property.parking_space,
...
function getDetailDecisionChips(property, features, transportation, limit = 6) {
    const chips = [];
    const text = buildDetailSearchText(property, features, transportation);
    const priceWan = parseWanAmount(property.price);
    const ageYears = parseAgeYears(property.age);
    const areaPing = parsePingAmount(property.total_area || property.area);
    const monthly = estimateDetailMonthlyPayment(property);

    function add(icon, text) {
        if (!text || chips.some(chip => chip.text === text)) return;
        chips.push({ icon, text });
    }

    if (priceWan > 0 && priceWan <= 1000) add('fa-sack-dollar', '千萬內');
    if ((priceWan > 0 && priceWan <= 1200) || /首購|低自備|成家|新婚|2房|兩房/.test(text)) add('fa-key', '首購友善');
    if (monthly) add('fa-calculator', `月付約 ${monthly}`);
    if (hasDetailParking(property, text)) add('fa-square-parking', /雙車/.test(text) ? '雙車位' : '有車位');
    if (ageYears !== null && ageYears <= 10) add('fa-building-circle-check', ageYears <= 3 ? '新屋齡' : `${ageYears}年內`);
    if (areaPing >= 45) add('fa-ruler-combined', '大坪數');
    if (/雙衛|2衛|兩衛/.test(text)) add('fa-bath', '雙衛');
    if (/龍科|幼獅/.test(text)) add('fa-location-dot', '龍科生活圈');
    if (/車站|火車站|近站|埔心|楊梅站/.test(text)) add('fa-train-subway', '近車站');
    if (/學區|國小|國中|高中|大學/.test(text)) add('fa-school', '學區機能');
    if (/商圈|市場|公園|交流道/.test(text)) add('fa-store', '生活機能');
...
function renderProperty(property) {
    ...
    const detailChips = getDetailDecisionChips(property, features, transportation, 6);
    const monthlyPayment = estimateDetailMonthlyPayment(property);
    const displayArea = property.total_area
        ? (property.total_area.includes('坪') ? property.total_area : property.total_area + '坪')
        : (property.area || '');
    const lineInquiryUrl = window.CustomerInquiry
        ? window.CustomerInquiry.buildLineInquiryUrl(property)
        : 'https://lin.ee/Lax7jMka';
    ...
    pageEl.innerHTML = `
        ...
        <div class="property-action-panel">
            <div class="property-action-summary">
                <div class="property-action-price">
                    <span>售價</span>
                    <strong>${property.price || '未設定'}</strong>
                </div>
                <div class="property-action-facts">
                    ${displayArea ? `<span><i class="fas fa-ruler-combined"></i>${displayArea}</span>` : ''}
                    ${property.layout ? `<span><i class="fas fa-door-open"></i>${property.layout}</span>` : ''}
                    ${monthlyPayment ? `<span><i class="fas fa-calculator"></i>${monthlyPayment}</span>` : ''}
                </div>
                ${detailChips.length ? `
                <div class="property-decision-chips">
                    ${detailChips.map(chip => `
                        <span><i class="fas ${chip.icon}"></i>${chip.text}</span>
                    `).join('')}
                </div>
                ` : ''}
            </div>
            <div class="property-action-buttons">
                <a class="property-action-btn call" href="tel:0928776755">
                    <i class="fas fa-phone"></i>
                    <span>立即來電</span>
                </a>
                <a class="property-action-btn line" href="${lineInquiryUrl}" target="_blank" rel="noopener" data-action="line-inquiry">
                    <i class="fa-brands fa-line"></i>
                    <span>LINE問房</span>
                </a>
                <button class="property-action-btn appointment" type="button" data-action="appointment">
        ...
        <div class="property-content">
            ...
            <div class="section">
                ...
                <div class="detail-map-shell">
                    ...
                    <div class="detail-map-overlay">
                        <div class="detail-map-card">
                            <span><i class="fas fa-location-dot"></i> 物件位置</span>
                            <strong>${displayAddress || property.address || '位置地圖'}</strong>
                            <em>${property.hide_address_number ? '地址已隱藏，顯示鄰近位置' : '可開啟 Google Maps 或規劃路線'}</em>
                        </div>
                    </div>
                </div>
                <div style="margin-top: 1rem; display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
                    ...
                    ` : `
                    <p class="detail-map-privacy-note">地址已隱藏，如需確切位置請聯絡我們。</p>
                    `}
                </div>
            </div>
            ` : ''}
            ...
            <div class="contact-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.8rem; max-width: 1200px; margin: 0 auto;">
                ...
                <div style="background: linear-gradient(135deg, #ffffff 0%, #f8f9fa 100%); padding: 1.8rem; border-radius: 15px; box-shadow: 0 8px 25px rgba(0,0,0,0.1); border: 1px solid rgba(102, 126, 234, 0.1); position: relative; overflow: hidden; display: flex; flex-direction: column; height: 100%;">
                    ...
                    <div style="display: flex; gap: 0.8rem; flex-wrap: wrap; margin-top: auto; padding-top: 1rem;">
                        ...
                        <button type="button" data-action="appointment" style="background: linear-gradient(45deg, #f59e0b, #ef4444); color: white; border: none; padding: 0.6rem 1.2rem; border-radius: 20px; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 0.4rem; transition: all 0.3s ease; box-shadow: 0 3px 12px rgba(239, 68, 68, 0.26); flex: 1 1 100%; justify-content: center; font-size: 0.85rem; cursor: pointer;"
                           onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 5px 15px rgba(239, 68, 68, 0.36)'"
                           onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 3px 12px rgba(239, 68, 68, 0.26)'">
                            <i class="bi bi-calendar-check-fill"></i> 預約看屋
                        </button>
                    </div>
                </div>
                <!-- 經紀業資訊卡片 -->
        ...
        <nav class="property-detail-mobile-cta" aria-label="物件快速聯絡">
            <a class="line" href="${lineInquiryUrl}" target="_blank" rel="noopener" data-action="line-inquiry">
                <i class="fa-brands fa-line"></i>
                <span>LINE問房</span>
            </a>
            <a class="call" href="tel:0928776755">
                <i class="fas fa-phone"></i>
                <span>來電</span>
            </a>
            <button class="appointment" type="button" data-action="appointment">
                <i class="fas fa-calendar-check"></i>
                <span>預約</span>
            </button>
        </nav>
...
// 載入相關連結（只渲染一次，避免詳情頁重複 fetch/render）
function loadRelatedLinksOnPage() {
    const containerId = 'related-links-container-detail';
    const maxRetries = 30;
    let retries = 0;
    let loaded = window.__relatedLinksDetailLoaded === true;

    function showRelatedLinksError() {
    ...
    function tryLoad() {
        ...
        if (hasModule) {
            loaded = true;
            window.__relatedLinksDetailLoaded = true;
            console.log('✅ 開始載入相關連結（詳情頁）');
            window.RelatedLinksFrontend.renderRelatedLinks(containerId).catch(error => {
                loaded = false;
                window.__relatedLinksDetailLoaded = false;
                console.error('❌ 載入相關連結失敗:', error);
                showRelatedLinksError();
            });
            return;
        }
    ...
    window.addEventListener('relatedLinksFrontendReady', tryLoad, { once: true });
    tryLoad();
}

...
### 2. Cloudflare Pages Functions（`property-detail.html.js` 與 `property-detail.js`）

- **`property-detail.html.js`**：只處理 **`/property-detail.html`** 的 GET。
- **`property-detail.js`**：只處理 **`/property-detail`**（無 .html）的 GET；許多分享連結使用此路徑，若無此 Function 預覽會只剩預設圖與文案。
- 從網址讀 `?number=xxx` 或 `?id=xxx`，向 Supabase 查詢該物件的 `images, title, price, address, hide_address_number, type`。
- **地址**：當 `hide_address_number` 為 true，或類型為透天／別墅／店面時，OG 描述只顯示遮罩後地址（例如路名），不露出完整門牌。
- **編號**：會嘗試正規化（如 `VI-002` → `VI00002`），提高與資料庫的查詢命中。
- 從 ASSETS 讀取靜態的 `property-detail.html`，把下面這些 meta 替換成該物件的實際內容後回傳（詳見下方「Meta 標籤一覽」）。

這樣一來，**`/property-detail` 與 `/property-detail.html`** 在**爬蟲**都會拿到已填入正確預覽（案名、售價、一張房屋照片、遮罩後地址）的 HTML。
...
1. **路徑對應**  
   - `property-detail.html.js` 對應 `/property-detail.html`；`property-detail.js` 對應 `/property-detail`（無 .html）。  
   - 分享或舊連結若使用 `/property-detail?number=xxx`，需有 `property-detail.js` 才會有正確預覽；若只有 `property-detail.html.js`，請改用 `/property-detail.html?number=xxx` 或新增 `property-detail.js`。  
   - 若網站有加 base path（例如 `/junyang666/`），要確認實際請求路徑是否仍為 `.../property-detail.html` 或 `.../property-detail`，必要時再調整 Function 的 route 或 ASSETS 路徑。
